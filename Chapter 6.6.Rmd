...
**I confess I struggle with this. We don't have strong guidance from Chapter 6 on what these values are expected to be. I will be interested to better understand expected values of these in nature / as correspond to biological hypotheses. For now, I put only a single trait impacting a single beta, and I gave those a small magnitude of impact (so that the niche isn't perfectly explained by the trait)**
...
```{r}
m = Hmsc(Y = Y, XData = XData, XFormula = ~x.1 + x.2, TrData = TrData,
TrFormula = ~trait.t2 + trait.t3, distr = "normal")
```
...
mcmc_intervals(m.df.gamma,point_size=1) + ggtitle("Gamma Posterior")
```

These aren't especially great, so I will need to read more about them.
